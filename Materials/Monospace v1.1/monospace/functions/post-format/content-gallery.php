		...
		<div class="latest-post-data">
			<div class="top-row">
				<?php if(isset($mts_options['mts_home_headline_meta']) == '1') { ?>
					...
					<?php } ?>
				<?php } ?>
				<span class="post-format-icon icon-picture"></span>				
			</div>
			...
			<div class="latest-post-img">
				<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>" rel="nofollow">
					<div class="slider-container loading">
						<div id="slider" class="small-slider">
							<ul class="slides">
							   <?php $i = 1;
							   foreach( $images as $image ){ $src = wp_get_attachment_image_src( $image->ID, 'large', false, '' );  ?>
							    	<li><?php echo wp_get_attachment_image($image->ID, 'featured3' ); ?></li>
							   <?php $i++; } ?>
							</ul>
						</div>
					</div>
